		...
		<div class="container mx-auto">
			<div class="px-6 py-6 bg-white rounded-lg shadow-md">
				<form v-if="globalShare.isLogined == false" v-on:submit.prevent="checkAndJoin">
					...
					<FormRow title="비밀번호확인">
						<input ref="loginPwConfirmElRef" class="form-row-input" type="password" placeholder="비밀번호 다시 입력해 주세요." />
					</FormRow>
					<FormRow title="이름">
				...
				<div v-else>이미 로그인 되었습니다. <route-link class="btn-link" to="/">홈</route-link> 으로 이동</div>
			</div>
		</div>
	</section>
</template>
	...
	export default defineComponent({
		...
		setup(props) {
			...
			function join(loginId: string, loginPw: string, name: string, nickname: string, cellphoneNo: string, email: string) {
				mainApi.member_doJoin(loginId, loginPw, name, nickname, cellphoneNo, email).then((axiosResponse) => {
