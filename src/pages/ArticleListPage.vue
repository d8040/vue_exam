			...
			<div class="mt-6" v-bind:key="article.id" v-for="article in state.articles">
				<div class="px-6 py-6 bg-white rounded-lg shadow-md">
					<div class="flex justify-between items-center">
						...
						<router-link :to="'list?boardId=' + article.boardId" class="px-2 py-1 bg-gray-600 text-gray-100 font-bold rounded hover:bg-gray-500">{{ article.boardId == 1 ? "공지" : "자유" }}</router-link>
					</div>
					<div class="mt-2">
						<router-link :to="'detail?id=' + article.id" class="text-2xl text-gray-700 font-bold hover:underline">{{ article.title }}</router-link>
						<p class="mt-2 text-gray-600">{{ article.body }}</p>
					</div>
					<div class="flex justify-between items-center mt-4">
						<router-link :to="'detail?id=' + article.id" class="text-blue-500 hover:underline">자세히 보기</router-link>
						<div>
