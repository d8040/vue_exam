<template>
	...
	<section class="section section-article-write-form px-2">
		<div class="container mx-auto">
			<div
				v-if="state.article.id !== undefined"
				class="flex my-10 bg-white shadow-md rounded-lg overflow-hidden mx-auto"
			>
						...
						<div class="flex flex-row mt-2 px-2 py-3 mx-3">
							...
							<div class="flex flex-col mb-2 ml-4 mt-1">
								...
								<div class="flex w-full mt-1">
									<router-link
										:to="'list?boardId' + state.article.boardId"
										class="text-blue-700 font-base text-xs mr-1 cursor-pointer"
									>{{ state.article.boardId == 1 ? "공지" : "자유" }}</router-link>
									<div class="text-gray-400 font-thin text-xs">{{ state.article.detail }}</div>
								</div>
							</div>
						</div>
...
export default defineComponent({
	...
	setup(props) {
		...
		function loadArticle(id: number) {
			mainApi.article_detail(id).then((axiosResponse) => {
